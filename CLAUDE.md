...
**Important:** high-volume agents (coder/tester/security) must use a direct `ANTHROPIC_API_KEY` or OpenRouter (pay-as-you-go) — heavy headless subscription usage hits the weekly caps and hard-stops mid-cycle. **Subscription-eligible roles:** the idea, planner, **reviewer**, and **escalate** slots may run on the operator's Claude Code subscription via the local `claude -p` CLI (`planner_agent/claude_code.py`; model `claude-code/sonnet|opus`, token from `claude setup-token` in `CLAUDE_CODE_OAUTH_TOKEN`). Planning is a handful of calls; the reviewer is one verdict call per PR — both fit subscription limits. `PATCH /api/config` allows `claude-code/*` only on those four roles and rejects it on coder/tester/security (the opencode coder can't resolve it and safely falls back to its base model; tester/security don't make the call). The reviewer runs in a sandbox, so `CLAUDE_CODE_OAUTH_TOKEN` is scoped into it (`sandbox.py` `_ROLE_ENV["reviewer"]`); the planner runs in-process and already has it. **Caveat:** pointing the reviewer at the subscription is an operator choice for when OpenRouter credit is unavailable — sustained use across many PRs can still hit weekly caps.
